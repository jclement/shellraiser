// Package assets holds everything `sr` needs to build a worker image locally:
// the base Dockerfile, the lean overlay template, the entrypoint, and the
// cross-compiled linux worker binaries. They are embedded so the single `sr`
...
// produced by `mise run build` and dropped in before `go build ./cmd/sr`. The
