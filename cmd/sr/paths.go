...
// with 0700 perms. Precedence: $SHELLRAISER_HOME → os.UserConfigDir()/shellraiser
// (~/.config/shellraiser on Linux, ~/Library/Application Support/shellraiser on macOS).
//
...
func globalDir() (string, error) {
	dir := os.Getenv("SHELLRAISER_HOME")
	if dir == "" {
		base, err := os.UserConfigDir()
		if err != nil {
			return "", fmt.Errorf("resolve config dir: %w", err)
		}
		dir = filepath.Join(base, "shellraiser")
	}
